    ...
    <div className="bg-white border-b border-slate-200 overflow-x-auto flex-shrink-0">
      ...
      <div className="flex">
        <div className="flex-1 px-4 py-2.5 border-r border-slate-200 min-w-max">
          <div className="text-xs text-slate-400 uppercase tracking-wide mb-0.5">Boards</div>
        ...
        <div className="flex-1 px-4 py-2.5 border-r border-slate-200 min-w-max">
          <div className="text-xs text-slate-400 uppercase tracking-wide mb-0.5">Pieces</div>
          <div className={`text-xl font-bold ${unplaced.length > 0 ? 'text-red-600' : 'text-slate-900'}`}>
            {result ? (unplaced.length > 0 ? `${placedCount}/${result.totalPieces}` : result.totalPieces) : '—'}
          </div>
        </div>
        <div className="flex-1 px-4 py-2.5 border-r border-slate-200 min-w-max">
          <div className="text-xs text-slate-400 uppercase tracking-wide mb-0.5">Efficiency</div>
          <div className={`text-xl font-bold ${effColor}`}>{result ? result.efficiency.toFixed(1) : '—'}%</div>
        </div>
        <div className="flex-1 px-4 py-2.5 border-r border-slate-200 min-w-max">
          <div className="text-xs text-slate-400 uppercase tracking-wide mb-0.5">Waste</div>
          <div className="text-xl font-bold text-amber-600">{wasteM2}m²</div>
        </div>
        <div className="flex-1 px-4 py-2.5 border-r border-slate-200 min-w-max">
          <div className="text-xs text-slate-400 uppercase tracking-wide mb-0.5">Offcuts</div>
          <div className="text-xl font-bold text-slate-900">{result ? result.usefulOffcuts : '—'}</div>
        </div>
        <div className="flex-1 px-4 py-2.5 min-w-max">
          <div className="text-xs text-slate-400 uppercase tracking-wide mb-0.5">Time</div>
